...
void node_tex_voronoi_smooth_distance_to_edge_2d(vec3 coord,
                                                 float w,
                                                 float scale,
                                                 float smoothness,
                                                 float exponent,
  ...
  for (int j = -2; j <= 2; j++) {
    for (int i = -2; i <= 2; i++) {
      ...
      if (dot(perpendicularToEdge, perpendicularToEdge) > 0.0001) {
        float distanceToEdge = dot((vectorToClosest + vectorToPoint) / 2.0,
                                   normalize(perpendicularToEdge));
        float h = smoothstep(0.0, 1.0, 0.5 + 0.5 * (minDistance - distanceToEdge) / smoothness);
        float correctionFactor = smoothness * h * (1.0 - h);
...
void node_tex_voronoi_smooth_distance_to_edge_3d(vec3 coord,
                                                 float w,
                                                 float scale,
                                                 float smoothness,
                                                 float exponent,
...
void node_tex_voronoi_smooth_distance_to_edge_4d(vec3 coord,
                                                 float w,
                                                 float scale,
                                                 float smoothness,
                                                 float exponent,
